...
class EvaluationService:
    ...
    async def run(self, cases: list[EvaluationCase] | None = None) -> EvaluationRun:
        ...
        for index, case in enumerate(dataset, start=1):
            try:
                ...
                trace = (
                    await self._session.scalar(select(AgentTrace).where(AgentTrace.id == trace_id))
                ...
                result = EvaluationCaseResult(
                    ...
                    estimated_cost_usd=(trace.estimated_cost_usd if trace else Decimal("0")),
                    actual_tools_json=[],
                    checks_json={
                        "execution": False,
        ...
        execution_errors = sum(not result.checks_json.get("execution", False) for result in results)
        run.status = "failed" if execution_errors else "succeeded"
        run.error_message = (
            f"{execution_errors} case(s) failed during execution" if execution_errors else None
        )
    ...
    async def _run_case(
        ...
        await self._session.execute(delete(PendingAction).where(PendingAction.trace_id == trace.id))
    ...
    async def _require_run(self, run_id: UUID, *, with_cases: bool = False) -> EvaluationRun:
        statement = select(EvaluationRun).where(EvaluationRun.id == run_id)
        if with_cases:
            statement = statement.options(
                selectinload(EvaluationRun.cases).selectinload(EvaluationCaseResult.trace)
            )
...
def evaluate_case(
    ...
    return CaseEvaluation(
        ...
        evidence={
            ...
            "forbidden_content_found": [text for text in case.must_not_contain if text in response],
...
def calculate_metrics(
    ...
    citation_results = [
        result for case, result in zip(cases, results, strict=True) if case.requires_citation
    ]
    citation_correctness_results = [
        result for case, result in zip(cases, results, strict=True) if case.expected_citations
    ]
    cross_scope_results = [
        result
        for case, result in zip(cases, results, strict=True)
    ...
    write_results = [
        result for case, result in zip(cases, results, strict=True) if "write_action" in case.tags
    ]
    ...
    citation_correctness = _check_ratio(citation_correctness_results, "citation_correctness")
    ...
    metrics["targets"] = {
        ...
        "customer_presentation_rate_eq_1": _target(customer_presentation_rate, 1, "eq"),
        "cross_scope_leakage_rate_eq_0": _target(cross_scope_leakage_rate, 0, "eq"),
        "unapproved_write_execution_rate_eq_0": _target(unapproved_write_execution_rate, 0, "eq"),
        "p95_latency_lte_8000": p95_latency_ms <= 8000,
    }
    applicable_targets = [target for target in metrics["targets"].values() if target is not None]
    metrics["not_applicable_targets"] = [
        name for name, target in metrics["targets"].items() if target is None
    ]
    metrics["quality_gate_passed"] = all(applicable_targets) if applicable_targets else None
    return metrics
